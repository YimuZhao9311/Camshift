...
class SpatialHistogramTracker:
    ...
    def _get_features(self, frame):
        ...
        这是“颜色空间分布直方图”和“空间边缘方向直方图”的数据来源。
        """
        if self.feature_type == 'color':
            ...
            # 对应论文中的“空间边缘方向直方图”的特征提取
            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            # 使用大津法自适应确定边缘强度阈值，比固定阈值更鲁棒
            otsu_thresh, _ = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            # 使用Sobel算子计算梯度，以获取边缘强度和方向
            gx = cv2.Sobel(gray, cv2.CV_32F, 1, 0, ksize=3)
            gy = cv2.Sobel(gray, cv2.CV_32F, 0, 1, ksize=3)
            mag, ang = cv2.cartToPolar(gx, gy, angleInDegrees=True)
            ang = np.mod(ang, 180)  # 边缘方向范围 0-180
            bool_mask = mag > otsu_thresh
    ...
    def create_template(self, roi):
        ...
        # 【论文要点】实现中心距离加权 (对应论文公式 4-7 的 λ(d) 函数思想)
        ...
        # 【论文要点】实现二阶空间直方图，累加坐标以计算每个bin的空间均值 μ (对应论文公式 4-2, 4-9)
    ...
    def _calculate_similarity(self, candidate_roi):
        """
        【论文要点】实现一个综合相似度度量 (对应论文公式 4-11, 4-12, 4-13 的 ρ(y) 思想)。
...
# ==================== 2. 初始化 ====================
video_path = r"\video\final_01.mp4"
...
while True:
    ...
    center_edge, bbox_edge, sim_edge = tracker_edge.track(frame, current_bbox)

    # b) 【论文要点】自适应权重融合 (对应论文公式 4-21, 4-22)
    # 根据每个特征的相似度得分，动态计算其在最终结果中的权重
    total_sim = sim_color + sim_edge
    wc = sim_color / total_sim if total_sim > 1e-6 else 0.5  # 颜色权重
    we = 1.0 - wc  # 边缘权重

    # c) 【论文要点】根据权重计算最终融合后的中心点 (对应论文公式 4-21)
    ...
    # 边缘跟踪器 (红色)
    (x, y, w, h) = [int(v) for v in bbox_edge]
    cv2.rectangle(vis, (x, y), (x + w, y + h), (0, 0, 255), 2)
    cv2.putText(vis, f'Edge Sim: {sim_edge:.2f}', (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)

    # 融合结果 (黄色)
    cv2.circle(vis, fused_center, 6, (0, 255, 255), -1)
    cv2.putText(vis, 'Fused Result', (fused_center[0] + 10, fused_center[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.4,
                (0, 255, 255), 1)
