...
class SpatialHistogramTracker:
    ...
    def _get_features(self, frame):
        ...
        if self.feature_type == 'color':
            ...
            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY); otsu_thresh, _ = cv2.threshold(gray, 0, 255,
                ...
                np.uint8) * 255; features = ang; return features, mask
    ...
    def _create_back_projection_map(self, frame, roi_bbox):
        ...
        if features is None:
            return np.zeros_like(roi[:, :, 0], dtype=np.float32) if len(roi.shape) > 2 else np.zeros_like(roi,
                                                                                                          dtype=np.float32)
        ...
        for r, c in zip(*coords):
            ...
            if prob_feature > 0:
                mu_model = self.model_hist[bin_idx][1]
                mu_cand = np.array([c, r])
                dist_sq = np.sum((mu_model - mu_cand) ** 2)
                prob_spatial = np.exp(-0.5 * dist_sq / (self.spatial_sigma ** 2))
...
# ==================== 2. 用户配置区和初始化 ====================
video_path = r"D:\PublicProject\HBY\zzy_camshift02\data\video\final_01.mp4"
gt_file_path = r"D:\PublicProject\HBY\zzy_camshift02\data\label\labeling_01_xml.csv"
...
while True:
    ...
    if not is_initialized:
        vis = frame.copy()
        cv2.putText(vis, f"Frame: {frame_idx} (Searching for GT)", (10, 30),
    ...
    cv2.putText(vis, 'Prediction', (x, y - 10),
    ...
    if frame_idx in gt_data:
        for _, gt_bbox in gt_data[frame_idx]:
            ...
            cv2.putText(vis, 'GroundTruth', (x_gt, y_gt - 10),
